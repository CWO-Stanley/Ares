<?php if($user->type == 2) { ?>
<?php
							$username = DB::Query("SELECT username, firstname, lastname, email, password, refresh  FROM users WHERE id = " . DB::Escape($_GET['id']));
...
<?php }else{ ?>
<?php
							$username = DB::Query("SELECT username, firstname, lastname, email, password, refresh  FROM users WHERE id = " . DB::Escape($_SESSION['id']));
							if(DB::NumRows($username) == 1) {
									$fetch = DB::Fetch($username);
							}
}
              ...
              <form class="form-horizontal" enctype="multipart/form-data" role="form" action="" method="Post">
                   ...
                   <!-- Text input-->
                    <div class="form-group">
					  ...
					  <?php }else{ ?>
					  <input id="userid" name="userid" type="text" placeholder="Gebruikersnaam" value="<?php echo $fetch['username'] ?>" class="form-control input-md" disabled>
					  <?php } ?>
