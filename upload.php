<?php
	...
	$image_name = strtolower($user->id) . 'nonstop.' . $extension;

						// $newname = 
	
	$name = $image_name;
	$result = move_uploaded_file($_FILES['file']['tmp_name'], 'avatars/'.$name);
	DB::Query("UPDATE users SET nonstopavatar='/".DB::Escape($name)."' WHERE id = '" . DB::Escape($user->id)."'");
	if($result){
		header('Location: nonstopsettings.php');
		}
